...
const ProfileScreen = () => {
  ...
  const handlePickImage = async () => {
    ImagePicker.launchImageLibrary({ mediaType: 'photo', quality: 0.7 }, async (response: ImagePickerResponse) => {
      ...
      if (response.assets && response.assets.length > 0) {
        ...
        if (asset.uri) {
          ...
          try {
            ...
            const updatedUser = { ...user, profilePic: data.profilePic || asset.uri };
            await AsyncStorage.setItem('user', JSON.stringify(updatedUser));
            setUser(updatedUser);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{ alignItems: 'center', marginBottom: 16 }}>
        {profilePic ? (
          <Image source={{ uri: profilePic }} style={{ width: 96, height: 96, borderRadius: 48, marginBottom: 8 }} />
        ) : (
        ...
        <Button title={uploading ? 'Uploading...' : 'Change Picture'} color="#607D8B" onPress={handlePickImage} disabled={uploading} />
      </View>
      {editMode ? (
        <>
          <TextInput
          ...
          <View style={{ flexDirection: 'row', gap: 12, marginTop: 12 }}>
            <Button title={saving ? 'Saving...' : 'Save'} color="#388E3C" onPress={handleSave} disabled={saving} />
            <Button title="Cancel" color="#888" onPress={() => { setEditMode(false); setPassword(''); setName(user.name); setEmail(user.email); }} />
          </View>
        </>
      ) : (
        <>
          ...
          <Button title="Edit Profile" color="#FF9800" onPress={() => setEditMode(true)} />
        </>
      )}
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 24, fontWeight: 'bold', marginBottom: 24, color: '#0a2342' },
  ...
  label: { fontWeight: 'bold', marginTop: 12, color: '#0a2342' },
  value: { fontSize: 16, marginBottom: 8, color: '#333' },
